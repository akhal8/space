...
if (isset($_POST['submit'])) {
    ...
    //         //check if name only contains letters and whitespace  
           if (!preg_match("/^[a-zA-Z ]*$/",$ast_name)) {  
            $errors['ast_name'] = 'An Input of characters with our without whitespaces is required';
         }
       }
    if (empty($_POST["ast_no_missions"])) {  
        $errors['ast_no_missions'] = 'An Input of into this field is required*';  
    } else {  
        $ast_no_missions = ($_POST["ast_no_missions"]);  
             //check if name only contains letters and whitespace  
             if (!preg_match("/^[a-zA-Z ]*$/",$ast_no_missions)) {  
              $errors['ast_no_missions'] = 'An Input of date is required';
         }
       }
